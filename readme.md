# Jingleplayer
## General information
This program's main application is to automate playing short audio files ("jingles") that are used as time signals at the start/end of each game of a sports tournament, but it can also do things like controlling music playback.

Automation is based on "events" (e.g. the games in your tournament, but depending on your needs this could also include things like the breaks inbetween) and "jingles" (a set of actions to be carried out a specific time relative to an event - e.g. at the start). Possible actions include:
- playing an audio file - e.g. a time signal to mark that the game is over
- pausing or resuming music playback (using Spotify or some other media player) - e.g. before or after a jingle is played to ensure that it isn't "drowned out" in music
- playing "announcements" for events - e.g. a pre-recorded message played before the final
- switching to a different Spotify playlist (or album, ...) - e.g. different music for games and breaks

The idea is that after setting up the configuration for a tournament (see [below](#usage)), you just need to start this program and (if you want) music playback at the beginning of the tournament. The program will then run unattended and no one needs to be time keeper.
...
- ... whether all events & jingles are set up properly;
...
## Basic configuration
Each tournament (i.e. a group of events for which you want to play the same jingles) is configured with a `.json` file. The [basic configuration file example](<tournaments/example/config - basic example.json>) is a good starting point, together with the notes/examples below.
...
### `timedelta`s
Multiple properties in the configuration file are `timedelta`s, for example the `duration` of a event or how far it is `offset` from another event. Here are some examples of valid inputs:
...
### Events
Events have a `start` and an `end`, both of which are `datetime` objects, meaning they refer to a specific time on a specific day. (Repeating events aren’t supported at the moment - you’ll need to create a separate event for each occurrence.) However, you don’t have to define both start and end explicitly: you can specify `start` or `end` relative to the start/end of another event and you can specify `start` and a `duration` instead of the `end`. See the following examples:
...
- relative `start` and `duration` instead of `end`:
  ...
  "events": {
    ...
    "Event 2": {
      "start": {
          "relative_to": "END OF EVENT: Event 1",
          "offset": "30min"
      },
      "duration": "1h"
    },
    "Event 3": {
      "start": {
        "relative_to": "END OF EVENT: Event 2",
        "offset": "30min"
      },
      "duration": "1h"
    }
...
If there are a lot of events in close succession, I recommend specify the first event's `start` explicitely and all other events relative to the previous one (like in the example above). The benefit is that you can easily re-schedule without having to edit all events by hand. For example, assume that after the first event, there is some unforeseen delay and the second event can not start at 13:30 as originally planned and needs to be shifted half an hour. Now, you can just close the program, change the configuration file to
...
and restart the program. Event 3 will automatically be adjusted as well.

Further notes:
- When defining a event’s `start` or `end` relative to another event, the referenced event must appear earlier in the list.
- Instead of `END OF EVENT: <name of event>`, you can also use `START OF EVENT: <name of event>`.
- You can also use `"relative_to": "NOW"` to make a event start or end relative to the current time (= when the program is started). This is mostly useful for debugging, I can't think of any real-world use case.

### Jingles
A jingle is a set of actions that is executed at a certain trigger time for each event. In the default configuration:
...
"Start" is played at the start of each event, "5min left" when there are 5 minutes left on the clock, and "End" at the end of each event.

If you want a specific part of your audio file to play exactly at the trigger time, adjust the `offset` accordingly. For example, if your “event is over” jingle has a 10-second countdown followed by a horn, you’ll probably want the horn to sound right when the event ends and not the countdown. To achieve this, you can use `"trigger" = "event_end"` with `"offset" = "-11s"`.

If the trigger time for a jingle/event combination has already passed when the program starts, it will simply be ignored. This lets you (re)start the program anytime during the tournament (even on the next day for multi-day tournaments) and the next jingle(s) will play at the correct time(s).
...
- announce each event (play a event-specific audio file),
...
- `announce event`: plays the current event's `announcement_file`. If none is set for this event, does nothing.
- `switch to event playlist`: switches Spotify playback to the current event's playlist. If none is set, does nothing. Note that switching to the playlist automatically starts playback if it was paused before, so there is no need to have a `resume playback` after this. See below for information on how to configure a playlist.
- `announce event playlist`: plays `announcement_file` of the current event's playlist. If the current event has no playlist set or the playlist has no announcement file set, does nothing.
...
"Start": {
    "audio_file": "jingle start.mp3",
    "trigger": "event_start",
    "pre_actions": "announce event; wait 2s",
    "actions": "play jingle"
},
```

If the event's announcement file is 5 seconds long, it will be played 7 seconds before the start of the event to ensure that the 2 second delay is kept and the jingle is played at the correct time.

Technical note: In reality, the scheduling might be off by a little bit due to two reasons:
- It is assumed that there is no overhead to execution, i.e. that `wait` takes exactly the specified time; `play jingle`, `announce event` and `announce event playlist` take exactly as long as the played audio file; and all other actions are instantenous.
...
- Larger features
  - Ability to set tags for events and jingle to only play (or to not play) for a specific tag. Use case examples:
    - Special jingles that should only play for the finale
    - Organizational announcements (lunch break, ...). Rename events to events for this?
